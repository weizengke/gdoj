	...
	public String userInfo() throws IOException{
        ...
        response.setCharacterEncoding("utf-8"); 
		 ...
		 if(user_.getAvatar()!=null&&user_.getAvatar().equals("Y")){
				buf+="<img style='width:80px;height:80px;' src='upload/userphoto/"+user_.getUsername()+"/photo.png' alt='"+user_.getUsername()+"'>";
			}else{
				buf+="<img style='width:80px;height:80px;' src='img/photo.png' alt='"+user_.getUsername()+"'>";
			}		
		 buf+="</a>";
		 buf+="</div>";
		
		 buf+="<h1><a href='mails/new/"+ user_.getUsername() + "' class='rated-user user-rate-"+ user_.getRate()+"' title='Mail to "+ getText("user_rate"+user_.getRate()) + " "+user_.getUsername() + "'>@"+user_.getUsername()+"</a></h1>" +
		 	"<span class='city'>";
		...
		"<li><a href='contests/" + user_.getUsername() +"'><em>"+user_.getRating()+"</em>"+getText("rating")+"</a></li>"+
		"<li><a href='problemset/status/with/"+user_.getUsername()+"/page/1'><em>"+user_.getSolved()+"</em>"+getText("solved")+"</a></li>"+	
		"<li style='border-right:0;'><a href='topics'><em>"+nCountTopic+"</em>"+getText("topic")+"</a></li>"+
		"</ul>"+
		"</div>"+
		"<div class='opts'>"+
